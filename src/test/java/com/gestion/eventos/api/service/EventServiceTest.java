...
class EventServiceTest {
    ...
    @Test
    @DisplayName("Dabe retornar un evento cuando el ID existe")
    ...
    void shouldSaveEventSuccessfullyWithoutSpeakers(){
        eventRequestDto.setSpeakersIds(new HashSet<>());
    ...
    void shouldThrowResourceNotFoundExceptionWhenCategoryNotFoundOnSave() {
        ...
        when(eventMapper.toEntity(any(EventRequestDto.class))).thenReturn(eventWithoutId);
        when(categoryService.findById(eventRequestDto.getCategoryId()))
                .thenThrow(new ResourceNotFoundException("Categoria no encontrada con id: " + eventRequestDto.getCategoryId()));
        ResourceNotFoundException thrown = assertThrows(ResourceNotFoundException.class, () -> {
            eventService.save(eventRequestDto);
        });

        assertEquals("Categoria no encontrada con id: " + eventRequestDto.getCategoryId(), thrown.getMessage());
