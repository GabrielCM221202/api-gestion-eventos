...
//Decirle a spring que vamos a testear el controller, para cargar el contexto web necesario
class EventControllerTest {
    ...
    void shouldReturnNotFoundWhenEventDoesNotExist() throws Exception{
        //Preparacion
        when(eventService.findById(anyLong())).thenThrow(new ResourceNotFoundException("Evento no encontrado con id: 99"));


        //Ejecucion
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/events/{id}", 99)
                .accept(MediaType.APPLICATION_JSON)
        )
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
